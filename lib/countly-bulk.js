...
/**
 * CountlyBulk object to manage the internal queue and send bulk requests to Countly server. More information on {@link http://resources.count.ly/docs/countly-sdk-for-nodejs}
 * @name CountlyBulk
 * @global
 * @namespace CountlyBulk
...
function CountlyBulk(conf){
    ...
    var initiated = false,
        ...
        failTimeout = 0,
        readyToProcess = true,
        requestQueue = storeGet("cly_req_queue", []),
    ...
    this.add_request = function(query){
        if(cluster.isMaster){
            ...
            if((query.timestamp + "").length !== 13 && (query.timestamp + "").length !== 10){
                log("incorrect timestamp format");
            }
    ...
    *   debug: true
    * });
    * server.add_bulk_request([{app_key:"somekey", devide_id:"someid", events:"[{'key':'val','count':1}]", begin_session:1}]);
    * @param {Array} bulk - array with multiple request objects that can be provided with {@link CountlyBulk.add_request}
    **/
    this.add_bulk request = function(requests){
        if(cluster.isMaster){
            var query;
        ...
        for(var device_id in eventQueue){
            if(eventQueue[device_id].length > 0){
                ...
                if(eventQueue[device_id].length <= conf.max_events){
                    self.add_request({device_id:device_id, events: JSON.stringify(eventQueue[device_id])});
                    eventQueue[device_id] = [];
                }
                else{
                    var events = eventQueue[device_id].splice(0, conf.max_events);
                    self.add_request({device_id:device_id, events: JSON.stringify(events)});
                }
            }
        }
        if(eventChanges){
