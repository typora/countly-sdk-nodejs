...
(function(Countly) {
  ...
  Countly.init = function(ob) {
    if (!inited) {
      ...
      failTimeoutAmount =
        ob.fail_timeout || Countly.fail_timeout || failTimeoutAmount;
      sessionUpdate =
        ob.session_update || Countly.session_update || sessionUpdate;
      ...
      Countly.storage_path =
        ob.storage_path || Countly.storage_path || "../data/";
      Countly.require_consent =
        ob.require_consent || Countly.require_consent || false;
  ...
  Countly.user_details = function(user) {
    if (Countly.check_consent("users")) {
      ...
      toRequestQueue({
        user_details: JSON.stringify(getProperties(user, props))
      });
    }
  };
  ...
  Countly.userData = {
    ...
    save: function() {
      if (Countly.check_consent("users")) {
        toRequestQueue({
          user_details: JSON.stringify({ custom: customData })
        });
      }
      customData = {};
    }
  ...
  var readFile = function(key) {
    var dir = path.resolve(
      __dirname,
      Countly.storage_path + "__" + key + ".json"
    );
  ...
  var forceStore = function() {
    for (var i in __data) {
      var dir = path.resolve(
        __dirname,
        Countly.storage_path + "__" + i + ".json"
      );
      var ob = {};
      ob[i] = __data[i];
      try {
